            ...
            <div class="col-10 d-flex justify-content-between h5">
                <?php
                    ...
                    for($j=0; $j<=7; $j++){
                            ...
                            $exist = $check->fetch();
                            //echo '<input type="radio" name="booking_time" value="' . $date->format('Y-m-d') . " " . $time_slot->format('H:i') .'" disabled><label>' . $time_slot->format('H:i') . '</label></td>';
                            echo '<input type="radio" name="booking_time" value="' . $date->format('Y-m-d') . " " . $time_slot->format('H:i') .'" ' . ($exist ? 'disabled' : '') . '><label>' . $time_slot->format('H:i') . '</label></td>';                            $time_slot=date_modify($time_slot, '+ 1 hour');
